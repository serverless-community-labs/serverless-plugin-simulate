...
describe('lambda proxy integration', () => {
  ...
  it('should pass through request bodies unchanged', () => {
    const postReq = cloneDeep(req)
    postReq.body = 'Foo=1&Bar=2&Baz=hello%2C%20world'
    postReq.headers['Content-Type'] = 'application/x-www-form-urlencoded'
    postReq.method = 'POST'
    const event = integration.event(postReq)
    expect(event.body).toBe(postReq.body)
  })
})
